...
func _ready():
	...
	if am:
		var p: AudioStreamPlayer = am.get_node_or_null("Level2")
		if p:
			if p.stream == null:
				push_warning("AudioManager/Level2 has no stream assigned.")
			else:
				p.stream.loop = true  # ทำให้ stream เล่นวนซ้ำ
				p.play()
		else:
			push_warning("Node 'Level2' not found under AudioManager.")
	else:
